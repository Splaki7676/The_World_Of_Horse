    ...
    if (pass.length === 0) {setError(passDiv, "Field is required."); return; }
    if (pass.length < 6) {setError(passDiv, "At least 6 characters."); return; }
            if (pass.length > 9) {setError(passDiv, "At most 9 characters."); return; }
    if (!/[0-9]/.test(pass)) {setError(passDiv, "Must contain a number."); return; }
    if (letterCount < 3) {setError(passDiv, "Must contain at least 3 letters."); return; }
